...
<script>
    ...
    function reason_y() {
        let reasonContent = $("#reason-content").val().trim()
        if (!reasonContent) {
            alert('이유를 적어주세요.')
            return false;
        }
        ...
        obj.approval_Cancel_Reason = reasonContent;
        obj.approval_Content = content;
        obj.doc_Id = docId;
        ...
        $.ajax({
            type: "post",
            url: "<%=request.getContextPath()%>/approval/approvalReturn",
    ...
    function review_go() {
        ...
        $.ajax({
            type: "POST",
            url: "<%=request.getContextPath()%>/approval/approvalReivew",
            contentType: "application/json",
            data: JSON.stringify(obj),
            async: false,
            processData: true,
