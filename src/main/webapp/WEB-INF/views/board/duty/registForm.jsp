...
<button type="button" class="btn" onclick="presentationFill_4();" style="position:absolute;right:1.5em;top:7.5em;color:#ffffff; z-index: 100; border: 0px;">시연용버튼4</button>
						...
						<div class="body">
							<form:form modelAttribute="boardFormVO" name="boardRegistForm">
								...
								<form:hidden path="dutyVO.duty_Board_Content"
									id="duty_Board_Content" />
								<form:hidden path="dutyVO.duty_Updater_Id"
									value="${loginUser.emp_Id }" />
								<form:hidden path="dutyVO.emp_Writer_Id"
									value="${loginUser.emp_Id }" />
	...
	<form name="issueRegistForm" method="post">
		<input type="hidden" name="issueBoardTitle" /> <input type="hidden"
			...
			name="issueBoardUpdaterId" value="${loginUser.emp_Id }" /> <input
			type="hidden" name="emp_Id" value="${loginUser.emp_Id }" /> <input
			type="hidden" name="duty_Board_Id" /> <input type="hidden"
			name="issueOpenSt" />
	</form>
...
window.onload = function(){
	...
	if(${!empty boardFormVO.registParam}){
		...
		if('${boardFormVO.registParam}' == 'issue'){
			$('#dutyBoardIdForIssue').val('${boardFormVO.duty_Board_Id}');
			$('#boardTitle').val('#${boardFormVO.duty_Board_Id}의 이슈 [${boardFormVO.dutyVO.duty_Board_Title}]');
			$('#summernote').summernote('code', '<br>'
					...
					+'<p>------------------------------</p>'
					+'<p>[업무제안 내용]</p>'
					+'${boardFormVO.dutyVO.duty_Board_Content}'
			);
		}
...
function submit_go(){
	...
	if(boardCategory=='duty'){	
		...
		for(var i = 0; i < $(".manager").length; i++){
			...
			var inputTag1 = '<input class="empManagerList" type="hidden" name="dutyVO.empManagerList" value="'+emp_Id+'"/>'
...
// 						sendMessage(contextPath, receiverId, alarmTitle, alarmContent, alarmUrl, alarmCategory)
						sendMessage('${pageContext.request.contextPath}',
									...
									$('#boardTitle').val()+' #'+data,
									'${pageContext.request.contextPath}/board/duty/detail?duty_Board_Id='+data,
									'업무관리');
					})(index);
				});
